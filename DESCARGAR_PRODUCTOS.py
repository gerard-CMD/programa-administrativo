...
from ventana_productos import InventoryWindow
from ventana_cargar_descargos import VentanaCargarTransaccionDescargo
from ventana_depositos import VentanaDepositos
import io  # Asegúrate de importar io al inicio del archivo
...
class VentanaPDF(QMainWindow):
    def __init__(self, pdf_buffer):
        super().__init__()
        self.setWindowTitle("Preliminar Descargos de Inventario")
...
class DescargarProductos(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def setupUi(self, MainWindow):
        self.setWindowTitle("Descargos de Productos")
        ...
        "                color: #ffffff;\n"
        "            }")
        self.tabla_productos.itemChanged.connect(self.filtrar_productos_por_codigo)
        ...
        self.tabla_productos.setColumnWidth(1, 554)
        self.tabla_productos.setColumnWidth(2, 40)
        self.tabla_productos.setColumnWidth(3, 47)
        ...
        pixmap = QPixmap("DESCARGOS.png")
        self.imagen_cargos.setPixmap(pixmap)
        self.imagen_cargos.setGeometry(QtCore.QRect(170, 10, 800, 55))
        ...
        self.button_producto.setGeometry(QtCore.QRect(145, 620, 107, 31))
        font = QtGui.QFont()
        font.setFamily("Montserrat SemiBold")
        ...
        self.button_guardar = QtWidgets.QPushButton(MainWindow)
        self.button_guardar.setGeometry(QtCore.QRect(358, 620, 102, 31))
        font = QtGui.QFont()
        font.setFamily("Montserrat SemiBold")
        ...
        "                background-color: #ccc; /* Color cuando el botón está deshabilitado */\n"
        "            }")
        self.button_guardar.clicked.connect(self.guardar_operacion)
        self.shortcut_guardar = QShortcut(QKeySequence("F4"), self)
        self.shortcut_guardar.activated.connect(self.guardar_operacion)

        self.button_cargar = QtWidgets.QPushButton(MainWindow)
        self.button_cargar.setGeometry(QtCore.QRect(260, 620, 91, 31))
        font = QtGui.QFont()
        font.setFamily("Montserrat SemiBold")
        self.button_cargar.setFont(font)
        self.button_cargar.setStyleSheet("QPushButton {\n"
        "                background-color: #008080; \n"
        "                color: #fff; \n"
        "                border: none; \n"
        "                border-radius: 5px; \n"
        ...
        "                background-color: #ccc; /* Color cuando el botón está deshabilitado */\n"
        "            }")
        self.button_cargar.clicked.connect(self.abrir_ventana_cargar)
        self.shortcut_cargar = QShortcut(QKeySequence("F3"), self)
        self.shortcut_cargar.activated.connect(self.abrir_ventana_cargar)
        ...
        self.shortcut_cancelar = QShortcut(QKeySequence("F5"), self)
    ...
    def retranslateUi(self, MainWindow):
        ...
        item.setText(_translate("Form", "CANT."))
        ...
        self.label_cargo.setText(_translate("Form", "DESCARGO N°"))
        ...
        self.button_guardar.setText(_translate("Form", "GUARDAR [F4]"))
        self.button_cargar.setText(_translate("Form", "CARGAR [F3]"))
        self.button_totalizar.setText(_translate("Form", "TOTALIZAR [F6]"))
        self.button_cancelar.setText(_translate("Form", "CANCELAR [F5]"))
    ...
    def abrir_ventana_productos(self):
        self.ventana = InventoryWindow(ventana_cargar=self, fila_actual=self.fila_actual)  # Pasa la ventana CargarProductos como parámetro
        self.ventana.setWindowModality(QtCore.Qt.ApplicationModal)
        self.ventana.show()

    def abrir_ventana_cargar(self):
        self.ventana = VentanaCargarTransaccionDescargo(self)  # Pasa la ventana CargarProductos como parámetro
        self.ventana.setWindowModality(QtCore.Qt.ApplicationModal)
        self.ventana.show()
    ...
    def filtrar_productos_por_codigo(self, item):
        if item.column() == 0:  # Solo si se editó la columna 0
            codigo = item.text()
            if codigo:  # Si se ingresó un código
                conn = sqlite3.connect('Usuarios.db')
                cursor = conn.cursor()
                ...
                if producto:  # Si se encontró el producto
                    self.tabla_productos.setItem(item.row(), 1, QtWidgets.QTableWidgetItem(producto[0]))  # Cargar la descripción
                    costo_actual = float(producto[1])
                    costo_actual_referencial = float(producto[2])

                    # Crear el widget personalizado para los resultados en la columna 4
                    widget_resultado_4 = QWidget()
                    layout_4 = QVBoxLayout(widget_resultado_4)

                    label1_4 = QLabel("{:,.2f}".format(costo_actual))
                    label1_4.setStyleSheet("background-color: #ffffff; padding: 2px;")
                    label1_4.setAlignment(QtCore.Qt.AlignRight)
                    layout_4.addWidget(label1_4)

                    label2_4 = QLabel("{:,.2f}".format(costo_actual_referencial))
                    font = QtGui.QFont()
                    font.setFamily("Montserrat SemiBold")
                    label2_4.setFont(font)
                    label2_4.setStyleSheet("background-color: #d5d9d7; padding: 2px;")
                    label2_4.setAlignment(QtCore.Qt.AlignLeft)
                    layout_4.addWidget(label2_4)

                    layout_4.setContentsMargins(0, 0, 0, 0)
                    layout_4.setSpacing(1)
                    
                    # Establecer el widget en la celda de la columna 4
                    self.tabla_productos.setCellWidget(item.row(), 4, widget_resultado_4)

                    self.tabla_productos.setRowHeight(item.row(), 35)
                    self.tabla_productos.editItem(self.tabla_productos.item(item.row(), 3))  # Editar la celda automáticamente
                
                else:  # Si no se encontró el producto, filtra los productos en la ventana VentanaProductos
                    self.ventana = InventoryWindow(ventana_cargar=self, fila_actual=item.row())  # Crea una instancia de la ventana
                    self.ventana.setWindowModality(QtCore.Qt.ApplicationModal)
                    self.ventana.filtrar_productos_por_codigo(codigo)  # Filtra los productos por código
    ...
    def itemChanged(self, item):           
        if item.column() == 3:  # Solo si se cambió el valor de la columna 3
            if item.text() != '':  # Verifica si la celda tiene un valor
                ...
                widget = self.tabla_productos.cellWidget(item.row(), 4)
                if widget:
                    layout = widget.layout()
                    label1 = layout.itemAt(0).widget()
                    label2 = layout.itemAt(1).widget()
                    monto1 = float(label1.text().replace(',', ''))
                    monto2 = float(label2.text().replace(',', ''))
                    resultado1 = round(cantidad * monto1, 2)
                    resultado2 = round(cantidad * monto2, 2)

                    # Crear el widget personalizado para los resultados en la columna 5
                    widget_resultado_5 = QWidget()
                    layout_5 = QVBoxLayout(widget_resultado_5)

                    label1_5 = QLabel("{:,.2f}".format(resultado1))
                    label1_5.setStyleSheet("background-color: #ffffff; padding: 2px;")
                    label1_5.setAlignment(QtCore.Qt.AlignRight)
                    layout_5.addWidget(label1_5)

                    label2_5 = QLabel("{:,.2f}".format(resultado2))
                    font = QtGui.QFont()
                    font.setFamily("Montserrat SemiBold")
                    label2_5.setFont(font)
                    label2_5.setStyleSheet("background-color: #d5d9d7; padding: 2px;")
                    label2_5.setAlignment(QtCore.Qt.AlignLeft)
                    layout_5.addWidget(label2_5)

                    layout_5.setContentsMargins(0, 0, 0, 0)
                    layout_5.setSpacing(1)
                
                    # Establecer el widget en la celda de la columna 5
                    self.tabla_productos.setCellWidget(item.row(), 5, widget_resultado_5)
                else:
                    self.tabla_productos.setItem(item.row(), 5, QTableWidgetItem(''))  # Establece la celda en blanco si no hay valor
            else:
                self.tabla_productos.setItem(item.row(), 5, QTableWidgetItem(''))  # Establece la celda en blanco si no hay valor
    ...
    def calcular_total_referencial(self):
        ...
        for row in range(self.tabla_productos.rowCount()):
            ...
            if widget:
                layout = widget.layout()
                label2 = layout.itemAt(1).widget()
                total += float(label2.text().replace(',', ''))
        return total
    ...
    def guardar_operacion(self):
        ...
        numero_operacion = self.obtener_siguiente_numero_operacion()
        
        for row in range(self.tabla_productos.rowCount()):
            codigo = self.tabla_productos.item(row, 0).text()
            cantidad = self.tabla_productos.item(row, 3).text()
            estado = "Guardada"  # O cualquier estado que desees

            # Verificar que el código y la cantidad no estén vacíos
            if codigo and cantidad:  # Solo proceder si ambos tienen datos
                cantidad = int(cantidad)  # Convertir a entero

                conn = sqlite3.connect('Usuarios.db')
                cursor = conn.cursor()

                # Insertar un nuevo registro en la tabla operaciones_guardadas
                cursor.execute('''
                    INSERT INTO operaciones_descargas_guardadas (numero_operacion, codigo, cantidad, estado, fecha)
                    VALUES (?, ?, ?, ?, ?);
                ''', (numero_operacion, codigo, cantidad, estado, datetime.now().strftime("%Y-%m-%d")))
                conn.commit()
                conn.close()
    ...
    def obtener_siguiente_numero_operacion(self):
        conn = sqlite3.connect('Usuarios.db')
        cursor = conn.cursor()
        cursor.execute('SELECT MAX(numero_operacion) FROM operaciones_descargas_guardadas;')
        result = cursor.fetchone()
        conn.close()
        
        if result[0] is not None:
            return result[0] + 1  # Incrementar el número máximo encontrado
        return 1  # Si no hay operaciones, comenzar desde 1

    def cargar_contador_cargos(self):
        conn = sqlite3.connect('Usuarios.db')
        cursor = conn.cursor()
        cursor.execute('SELECT descargo_inventario FROM correlativos WHERE descargo_inventario IS NOT NULL ORDER BY id DESC LIMIT 1;')
        result = cursor.fetchone()

        if result:  # Verificar que result no sea None y que el primer elemento no sea None
            self.contador_cargos = int(result[0])
        else:
            self.contador_cargos = 1  # Valor por defecto si no se encuentra o si es None
    ...
    def actualizar_contador_cargos(self):
        ...
        cursor.execute('''
            INSERT INTO correlativos (descargo_inventario) VALUES (?)
            ON CONFLICT(id) DO UPDATE SET descargo_inventario = excluded.descargo_inventario;
    ...
    def totalizar(self):
        ...
        descargo_numero = f"{contador_cargos:08d}"
        ...
        for row in range(self.tabla_productos.rowCount()):
            ...
            cantidad = self.tabla_productos.item(row, 3).text()  # Cantidad

            if codigo_producto and cantidad:  # Verificar que el código y la cantidad no estén vacíos
                cantidad = int(cantidad)  # Convertir a entero
                ...
                # Validar que la cantidad a restar no exceda la cantidad actual
                if cantidad_actual >= cantidad:
                    nueva_cantidad = cantidad_actual - cantidad
                    cursor.execute('''UPDATE existencia_productos
                        SET cantidad = ?, fecha_registro = ?
                        WHERE codigo = ? AND deposito = ?''', (nueva_cantidad, datetime.now().strftime("%Y-%m-%d"), codigo_producto, deposito_id))
                else:
                    QMessageBox.warning(self, "Error", "No hay suficiente cantidad en el inventario.")
        ...
        pdf_filename = self.generar_pdf(descargo_numero)
        ...
        self.registrar_carga(descargo_numero, self.campo_autorizado.text())
    ...
    def generar_pdf(self, cargo_numero):
        ...
        carpeta_destino = os.path.join(directorio_script, "Descargos_inventario")

        os.makedirs(carpeta_destino, exist_ok=True)  # Crear la carpeta si no existe

        # Crear el archivo PDF
        pdf_filename = os.path.join(carpeta_destino, f"descargo_inventario_{cargo_numero}.pdf")  # Ruta completa
        ...
        if logo_path:
            ...
            view_width = 170  # Ajusta el ancho según sea necesario
        ...
        # Título del PDF
        titulo = Paragraph("Descargos de Inventario", styles['Title'])
        story.append(titulo)
        story.append(Spacer(1, 12))

        # Crear una tabla para el factor y la fecha/hora
        data_info = [
            [f"Depósito: {self.campo_codigo_deposito.text()} - {self.campo_deposito.text()}", f"Número de Descargo: {self.contadors_cargo.text()}"]
        ]

        # Crear la tabla
        info_table = Table(data_info, colWidths=[410, 160])
        ...
        # Crear la tabla
        info_table = Table(data_info, colWidths=[100, 470])
        info_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, -1), colors.white),
            ('TEXTCOLOR', (0, 0), (-1, -1), colors.black),
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ...
        # Crear la tabla
        info_table = Table(data_info, colWidths=[370, 200])
        info_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, -1), colors.white),
            ('TEXTCOLOR', (0, 0), (-1, -1), colors.black),
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ...
        data = [["Código", "Descripción", "Cantidad", "Costo", "Total"]]
        for row in range(self.tabla_productos.rowCount()):
            codigo = self.tabla_productos.item(row, 0)
            if codigo and codigo.text().strip():
                ...
                # Obtener el widget de la columna 5 (Total)
                widget = self.tabla_productos.cellWidget(row, 4)
                if widget:
                    layout = widget.layout()
                    label1 = layout.itemAt(0).widget()  # Primer QLabel (Total en Bs.)
                    label2 = layout.itemAt(1).widget()  # Segundo QLabel (Total en $)
                    costo_bs = label1.text()
                    costo_usd = label2.text()
                ...
                if widget:
                    ...
                    label1 = layout.itemAt(0).widget()
                    label2 = layout.itemAt(1).widget()
                    total_bs = label1.text()
                    total_usd = label2.text()
                        
                    data.append([codigo.text(), descripcion, cantidad, f'{costo_bs}\n{costo_usd}', f'{total_bs}\n{total_usd}'])

        # Crear la tabla
        tabla = Table(data, colWidths=[80, 300, 50, 50, 80])
        ...
        if table_height < doc.height - 100:  # Ajusta el valor según sea necesario
            ...
            total = self.label_total.text().split(":")[-1].strip()
            total_referencial = self.label_total_referencial.text().split(":")[-1].strip()

            # Crear datos para la tabla de totales
            totales_data = [
                [f"Total Ítem: {total_item}", "Total Bs:", total],
                [f"Total Cantidad: {total_cantidad}", "Total $:", total_referencial],
            ]
    ...
    def registrar_carga(self, numero_cargo, autorizado):
        ...
        cursor.execute('''
            INSERT INTO historial_descargas (numero_cargo, autorizado)
            VALUES (?, ?)
    ...
    def preliminar_operacion(self):
        ...
        # Agregar un Spacer para crear espacio entre la tabla de productos y la tabla de totales
        story.append(Spacer(1, 6))  # Espacio de 12 unidades
        ...
        # Crear una tabla para el factor y la fecha/hora
        data_info = [
        [f"Depósito: {self.campo_codigo_deposito.text()} - {self.campo_deposito.text()} ", f"Número de Descargos: {self.contadors_cargo.text()}"]
        ]
        ...
        # Agregar la tabla a la historia
        story.append(info_table)
        story.append(Spacer(1, 8)) # Espacio de 20 unidades
