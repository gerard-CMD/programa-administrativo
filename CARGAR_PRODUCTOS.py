...
from ventana_cargar import VentanaCargarTransaccion
...
class CargarProductos(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def setupUi(self, MainWindow):
        ...
        pixmap = QPixmap("CARGOS.png")
        self.imagen_cargos.setPixmap(pixmap)
        self.imagen_cargos.setGeometry(QtCore.QRect(195, 10, 750, 55))
        ...
        self.button_producto.setGeometry(QtCore.QRect(60, 620, 107, 31))
        font = QtGui.QFont()
        font.setFamily("Montserrat SemiBold")
        ...
        self.button_cargar.setGeometry(QtCore.QRect(262, 620, 91, 31))
        font = QtGui.QFont()
        font.setFamily("Montserrat SemiBold")
        ...
        self.button_crear = QtWidgets.QPushButton(MainWindow)
        self.button_crear.setGeometry(QtCore.QRect(173, 620, 83, 31))
        font = QtGui.QFont()
        font.setFamily("Montserrat SemiBold")
        self.button_crear.setFont(font)
        ...
        "                background-color: #ccc; /* Color cuando el botón está deshabilitado */\n"
        "            }")
        self.button_crear.clicked.connect(self.abrir_ventana_crear)
        self.shortcut_crear = QShortcut(QKeySequence("F2"), self)
        self.shortcut_crear.activated.connect(self.abrir_ventana_crear)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.button_crear.setText(_translate("Form", "CREAR [F2]"))
    ...
    def abrir_ventana_cargar(self):
        self.ventana = VentanaCargarTransaccion(self)  # Pasa la ventana CargarProductos como parámetro
        self.ventana.setWindowModality(QtCore.Qt.ApplicationModal)
        self.ventana.show()

    def abrir_ventana_crear(self):
        self.ventana = CrearProductos()
        self.ventana.setWindowModality(QtCore.Qt.ApplicationModal)
        self.ventana.show()
    ...
    def guardar_operacion(self):
        ...
        for row in range(self.tabla_productos.rowCount()):
            ...
            if codigo and cantidad:  # Solo proceder si ambos tienen datos
                ...
                cursor.execute('''
                    INSERT INTO operaciones_guardadas (numero_operacion, codigo, cantidad, estado, fecha)
                    VALUES (?, ?, ?, ?, ?);
    ...
    def obtener_siguiente_numero_operacion(self):
        conn = sqlite3.connect('Usuarios.db')
        cursor = conn.cursor()
        cursor.execute('SELECT MAX(numero_operacion) FROM operaciones_guardadas;')
    ...
    def cargar_contador_cargos(self):
        conn = sqlite3.connect('Usuarios.db')
        cursor = conn.cursor()
        cursor.execute('SELECT cargo_inventario FROM correlativos WHERE cargo_inventario IS NOT NULL ORDER BY id DESC LIMIT 1;')
        result = cursor.fetchone()
        if result:
    ...
    def actualizar_contador_cargos(self):
        ...
        cursor.execute('''
            INSERT INTO correlativos (cargo_inventario) VALUES (?)
            ON CONFLICT(id) DO UPDATE SET cargo_inventario = excluded.cargo_inventario;
            ...
            if codigo_producto and cantidad:  # Verificar que el código y la cantidad no estén vacíos
                ...
                if resultado:  # Si el producto ya existe, actualizar la cantidad
                    ...
                    cursor.execute('''UPDATE existencia_productos
                        ...
                        WHERE codigo = ? AND deposito = ?''', (nueva_cantidad, cargo_numero, datetime.now().strftime("%Y-%m-%d"), self.campo_autorizado.text(), codigo_producto, deposito_id))
                    ...
                    cursor.execute('''INSERT INTO existencia_productos (codigo, deposito, cantidad, cargo_numero, fecha_registro, autorizado)
                        VALUES (?, ?, ?, ?, ?, ?)''', (codigo_producto, deposito_id, cantidad, cargo_numero, datetime.now().strftime("%Y-%m-%d"), self.campo_autorizado.text()))
    ...
    def generar_pdf(self, cargo_numero):
        ...
        # Título del PDF
        titulo = Paragraph("Cargos de Inventario", styles['Title'])
        story.append(titulo)
        ...
        data_info = [
        [f"Depósito: {self.campo_codigo_deposito.text()} - {self.campo_deposito.text()} ", f"Número de Cargos: {self.contadors_cargo.text()}"]
        ]
        ...
        if table_height < doc.height - 100:  # Ajusta el valor según sea necesario
            ...
            totales_data = [
                [f"Total Ítem: {total_item}", "Total:", total],
                [f"Total Cantidad: {total_cantidad}", "Referencial:", total_referencial],
            ]
    ...
    def registrar_carga(self, numero_cargo, autorizado):
        ...
        cursor.execute('''
            INSERT INTO historial_cargas (numero_cargo, autorizado)
            VALUES (?, ?)
